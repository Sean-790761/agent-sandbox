...
func (r *SandboxReconciler) reconcileService(ctx context.Context, sandbox *sandboxv1beta1.Sandbox, nameHash string) (*corev1.Service, error) {
	...
	if err := r.Get(ctx, types.NamespacedName{Name: sandbox.Name, Namespace: sandbox.Namespace}, service); err != nil {
		...
		if desired != nil && *desired {
			...
			if err != nil {
				if k8serrors.IsInvalid(err) {
					// Permanent misconfiguration (e.g. the Sandbox name yields a
					// Service name over Kubernetes' 63-character limit). Retrying
					// cannot succeed, so log at a lower level and let the caller
					// surface it via the Ready=False/InvalidConfiguration condition
					// instead of requeuing on an error-level stacktrace.
					logger.Info("Refusing to create Service: invalid configuration",
